...
def generate_drums():
    ...
    try:
        ...
        print(f"[DrumGen] Provided genres (raw): {genres if genres is not None else 'None'}")
        print(f"[DrumGen] Provided primary_genre (raw): {primary_genre if primary_genre is not None else 'None'}")
        ...
        print(f"[DrumGen] Normalized genres: {provided_genres if provided_genres is not None else 'None'}")
        print(f"[DrumGen] Normalized primary_genre: {normalized_primary_genre if normalized_primary_genre is not None else 'None'}")

        _report_status(task_id, "Определение жанров...")
        if progress_delay_seconds > 0:
            time.sleep(progress_delay_seconds)
        print(f"[DrumGen] Generating notes | BPM: {bpm}, Lanes: {lanes}, Mode: {drum_mode}")
        print(f"[DrumGen] Generator params: use_stems=True, use_madmom_beats=True, use_filename_for_genres=False, auto_identify_track=False")
        print(f"[DrumGen] Passing genres to generator: {provided_genres if provided_genres is not None else 'None'}; primary_genre: {normalized_primary_genre if normalized_primary_genre is not None else 'None'}")
