...
def gpu_status_payload() -> Dict[str, object]:
    """Compact GPU status for /health and UI."""
    mode = _env_mode()
    ...
    return {
        "rfall_gpu": mode,
        "stems": stem_label,
        "stems_detail": stem_detail,
        "adtof": adtof_label,
        "adtof_detail": adtof_detail,
    }


def startup_gpu_message() -> str:
    info = gpu_status_payload()
    return (
        f"[Startup] GPU: stems={info.get('stems_detail')}, "
        f"ADTOF={info.get('adtof_detail')}, RFALL_GPU={info.get('rfall_gpu')}"
    )
