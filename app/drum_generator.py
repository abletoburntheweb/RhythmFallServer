...
def _apply_critic_intro_no_add(
    ...
    intro_scope = [
        t for t in times
        if drum_section_start - tol <= t < intro_end
    ]
    intro_buckets = _times_per_measure(intro_scope, beats, bpm) if intro_scope else {}
    earliest_pool = None
    if preserve_first and match_pool:
        # Match recovery grace: first onset may sit ~0.5s before section_start.
        lookback = max(beat_interval * grace_beats, float(preset.get("drum_entry_grace_sec", 0.65) or 0.65))
        after_start = [c for c in match_pool if c >= drum_section_start - lookback]
        if after_start:
    ...
    for t in sorted(times):
        if t < intro_end:
            ...
            if (
                ...
                and t >= drum_section_start - tol * 0.5
                and abs(t - earliest_pool) <= tol * 1.5
            ):
                kept.append(t)
            ...
            matched = any(
                abs(t - c) <= tol
                for c in match_pool
                if c >= drum_section_start - beat_interval * 0.5
            )
            if matched:
                kept.append(t)
            else:
...
def _recover_drum_entry_hits(
    ...
    max_add = int(preset.get("drum_entry_max_recover", 3) or 3)
    merge_tol = float(preset.get("drum_entry_merge_tol", 0.04) or 0.04)

    window_start = max(0.0, drum_section_start - grace_before)
    window_end = drum_section_start + entry_span
    recap["window"] = (window_start, window_end)
    ...
    # Prefer the earliest kick/snare just before section_start (the classic miss).
    pre_hits = [t for t in core_pool if window_start <= t < drum_section_start]
    in_window = [t for t in core_pool if drum_section_start <= t < window_end]
    candidates = sorted(pre_hits + in_window)
    ...
    if verbose:
        print(
            ...
            f"window=[{window_start:.3f},{window_end:.3f}) grace={grace_before:.3f}s "
            f"recovered=+{len(added)} times={[round(t, 3) for t in added]}"
        )
...
def generate_drums_notes(
    ...
    if verbose:
        entry_note = ""
        if len(filtered_events) != before_entry:
            entry_note = f" entry_recover={before_entry}->{len(filtered_events)}"
        print(
    ...
    if use_ergonomic and classified_hits:
        notes = assign_lanes_ergonomic(
            all_events, classified_hits, lanes=lanes, song_offset=0.0, bpm=bpm
        )
        if verbose:
