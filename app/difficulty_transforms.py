...
# North star Skeleton (PR1): kick+snare. Main crashes / tom fills → later PR.
SKELETON_CLASSES = CORE_CLASSES
...
def is_skeleton_hit(
    ...
    *,
    ...
    return drum in SKELETON_CLASSES
...
def trim_to_event_ratio(
    ...
    *,
    ...
    Skeleton (kick/snare) is never dropped — Arcade north star.
    """
    if not events:
        return [], 0
    ...
    skeleton = [
        float(t)
        for t in events
        if is_skeleton_hit(float(t), classified_hits, tolerance=tolerance)
    ]
    non_skeleton = [
        float(t)
        for t in events
        if not is_skeleton_hit(float(t), classified_hits, tolerance=tolerance)
    ]
    # Always keep full skeleton; fill remaining budget from priority-ranked texture/groove.
    room = max(0, target_count - len(skeleton))
    scored = [
        (_event_priority(t, classified_hits, tolerance), t)
        for t in non_skeleton
    ]
    scored.sort(key=lambda item: (-item[0], item[1]))
    kept_extra = [t for _prio, t in scored[:room]]
    kept = sorted(set(skeleton) | set(kept_extra))
...
def sparsify_by_beats(
    ...
    *,
    ...
    """Keep at most max_per_beat events per beat interval.

    When classified_hits are available, skeleton (kick/snare) is preferred and
    all skeleton hits in the beat are kept even if that exceeds max_per_beat
    (e.g. kick+snare flam on one beat). Non-skeleton fills remaining slots.
    """
    if not events:
        return []
    if beats is not None and len(beats) >= 2:
        ...
        for b in range(len(beats) - 1):
            ...
            if not bucket:
                continue
            if classified_hits:
                skel = [t for t in bucket if is_skeleton_hit(t, classified_hits, tolerance=tolerance)]
                other = [
                    t for t in bucket
                    if not is_skeleton_hit(t, classified_hits, tolerance=tolerance)
                ]
                # Never drop skeleton on a beat; fill remaining slots with groove/texture.
                room = max(0, int(max_per_beat) - len(skel))
                sparse.extend(skel)
                sparse.extend(other[:room])
            else:
                taken = 0
                for t in bucket:
...
def apply_difficulty_transform(
    ...
    *,
    ...
    Original vs Arcade share difficulty *labels* but not the same transforms:
    Original is documentary (keep detector material); Arcade is playable thinning.
    See docs/generation_settings_screen.md matrix.
    """
    ...
    if goal_n == "original":
        # Documentary axis — do not run Arcade playability thins (ghost wipe / 16→8 / ratio).
        if diff == "relaxed":
            ...
            out = sparsify_by_beats(
                out,
                beats,
                bpm,
                max_per_beat=max_per_beat,
                classified_hits=classified_hits,
            )
            ...
            # standard / default bake: ordinary documentary — preserve style map.
            recap["documentary_passthrough"] = True
    elif diff == "relaxed":
        out, q_merged = quantize_fast_runs(out, beats=beats, bpm=bpm)
        recap["quantize_fast_runs"] = q_merged
        out, ghosts = strip_ghost_notes(
