...
def _kuielab_mdx_segment_size(model_dir: Path) -> int:
    """kuielab_a_drums uses mdx_dim_t_set=9 (512). Default separator segment_size=256 skips ONNX+DirectML."""
    default = 512
    try:
        onnx_path = model_dir / "kuielab_a_drums.onnx"
        ...
        entry = payload.get("mdx_model_data", {}).get(digest, {})
...
def _separator_mdx_params(model_dir: Path) -> Dict:
    segment_size = _kuielab_mdx_segment_size(model_dir)
    return {
        "hop_length": 1024,
...
def _pick_stem_model_name(available_models: List[str], stem_type: str) -> Optional[str]:
    ...
    if mode in ("htdemucs", "demucs"):
        ...
        # kuielab / mdx — как в тестах: kuielab_a_drums, fallback htdemucs
        target_model = _match_kuielab_model(available_models, stem_type)
        if not target_model:
            target_model = _match_htdemucs_model(available_models)
...
def _create_separator(splitter_folder: Path, use_directml: bool, stem_type: str = "drums") -> "Separator":
    model_dir = str(LOCAL_MODELS_DIR) if LOCAL_MODELS_DIR.exists() else "/tmp/audio-separator-models/"
    mdx_params = _separator_mdx_params(Path(model_dir))
    return Separator(
        ...
        try:
            ...
            if warm is not None:
                ...
                print(
                    ...
                    f"mdx_segment_size={mdx_params['segment_size']})"
                )
                separator = _create_separator(splitter_folder, use_directml, stem_type)
                available_models: List[str] = []
                try:
                    available_models = separator.get_simplified_model_list()
                except Exception:
