...
GOAL_DIFF_BASE_PRESET: Dict[str, str] = {
    "original|easy": "minimal",
    "original|medium": "basic",
    "original|hard": "basic",
    "arcade|easy": "minimal",
    "arcade|medium": "basic",
    "arcade|hard": "enhanced",
}
...
_GOAL_POLICY: Dict[str, Dict[str, Any]] = {
    "original": {
        ...
        # Documentary: keep detector density; BPM sixteenth-cap still applies in cluster/flam.
        "hit_cluster_window": 0.06,
        "hit_cluster_beat_fraction": 0.20,
        "flam_merge_sec": 0.055,
        # Intro strip fights drum_entry + grid snap on real preamble hits (TTFAF m0/m2).
        "critic_intro_no_add": False,
        # Keep sparse intro hits before dense groove (section_start trim is Arcade-oriented).
        "keep_pre_section_hits": True,
    },
    "arcade": {
        ...
        # Sparse restraint: do not densify quiet/single-hit bars via add passes.
        "arcade_sparse_restraint": True,
        "arcade_sparse_max_notes": 2,
        "arcade_add_tension_min": 0.28,
        "arcade_sparse_vs_neighbor_ratio": 0.5,
        # Phrase lane layouts (4 bars → mirror/rotate variant).
        "arcade_lane_phrase_bars": 4,
        "arcade_lane_phrase_mode": "mirror",
        "arcade_lane_phrase_bias": 1.25,
    },
}
...
# Arcade easy: lighter completion than standard arcade.
_ARCADE_EASY_OVERRIDES: Dict[str, Any] = {
    "groove_completion": True,
...
# Original hard: keep documentary policy but allow more detector output.
_ORIGINAL_HARD_OVERRIDES: Dict[str, Any] = {
    "groove_completion": False,
    "loop_reinforce": False,
    "loop4_reinforce": False,
    "fill_recover": False,
...
def normalize_difficulty(value: Optional[str]) -> str:
    key = str(value or "").strip().lower()
    legacy = {"relaxed": "easy", "standard": "medium", "dense": "hard"}
...
def chart_stem_for_pair(goal: str, difficulty: str) -> str:
    ...
    # Original is one documentary chart → stem "original" (not original_medium).
    if g == "original":
        return "original"
...
def pair_from_stem(stem: str) -> Dict[str, str]:
    ...
    if key == "original" or key.startswith("original_"):
        if key.startswith("original_"):
            ...
            return {"goal": "original", "difficulty": normalize_difficulty(legacy_d)}
        return {"goal": "original", "difficulty": DEFAULT_DIFFICULTY}
    if key.startswith("arcade_"):
        token = key[len("arcade_") :].split("_", 1)[0]
        return {"goal": "arcade", "difficulty": normalize_difficulty(token)}
...
CHART_INTENT_TO_GOAL_DIFF: Dict[str, Tuple[str, str]] = {
    "original": (DEFAULT_GOAL, "medium"),
    "groove": ("arcade", "medium"),
    "sparse": (DEFAULT_GOAL, "easy"),
    "arcade": ("arcade", "medium"),
}
...
def resolve_goal_difficulty(
    *,
    ...
    if goal_n == "arcade" and diff_n == "easy":
        preset.update(_ARCADE_EASY_OVERRIDES)
    if goal_n == "arcade":
        # North star PR1: never let caps/cluster eat kick/snare skeleton.
        preset["preserve_core_hits"] = True
        if diff_n == "hard":
            # Dense keeps more texture; skip aggressive hat/cymbal run downsample.
            preset["arcade_texture_downsample"] = False
    if goal_n == "original" and diff_n == "hard":
        preset.update(_ORIGINAL_HARD_OVERRIDES)
    if goal_n == "original" and diff_n == "medium":
        # Ordinary documentary: do not inherit Arcade standard ratio trim / playability caps.
        # TTFAF dense 16ths need >12 npm; binding basic caps caused MISSING_NOTES (P2_thin).
        preset["difficulty_event_ratio"] = 0.0
        preset["max_notes_per_measure"] = 0
        preset["max_hits_per_second"] = 0
    if goal_n == "original" and diff_n == "hard":
        preset["difficulty_event_ratio"] = 0.0
        preset["max_notes_per_measure"] = 0
        preset["max_hits_per_second"] = 0
    if goal_n == "original" and diff_n == "easy":
